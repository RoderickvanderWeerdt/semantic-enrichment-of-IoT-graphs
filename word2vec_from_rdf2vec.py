...
@attr.s(init=False)
class Word2Vec_Epochs(Embedder):
    """Defines the Word2Vec embedding technique.
    ...
    def fit(
        ...
        self._models = []
        # for i in range(0, self._model.epochs):
        # for i in [0, 1, 2, 3, 4, 10, 20, self._model.epochs]:
        for i in range(0, self._model.epochs):
        # for i in [self._model.epochs-1]:
            self._models.append(copy.deepcopy(self._model))
            self._models[-1].epochs = i+1

        for i in range(0, len(self._models)):
            print("start training model for", self._models[i].epochs, "epochs...")
            self._models[i].train(
                corpus,
                total_examples=self._models[i].corpus_count,
                epochs=self._models[i].epochs,
            )
        return self
    ...
    def transform(self, entities: Entities) -> Embeddings:
        ...
        """
        if not all([entity in self._models[0].wv for entity in entities]):
            raise ValueError(
                "The entities must have been provided to fit() first "
                "before they can be transformed into a numerical vector."
            )
        model_vectors = []
        for i in range(0, len(self._models)):
            model_vectors.append([self._models[i].wv.get_vector(entity) for entity in entities])

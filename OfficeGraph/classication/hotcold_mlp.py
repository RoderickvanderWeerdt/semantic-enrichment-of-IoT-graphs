...
def perform_prediction(dataset_fn, show_all, save_model=False, embeddings_or_features=1, use_all=False): #embeddings=1, features=2, both=3
    if not use_all:
        shuffle_dataset(dataset_fn)
        if embeddings_or_features == 1:
            training_data = Emb_Workhours_Dataset(csv_file="shuffled_dataset.csv", train=True, transform=ToTensor())
            test_data = Emb_Workhours_Dataset(csv_file="shuffled_dataset.csv", train=False, transform=ToTensor())
